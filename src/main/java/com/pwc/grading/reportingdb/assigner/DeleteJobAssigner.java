...
//					ExecutorServiceConfig.getExecutorService().submit(reportDatabaseJob);
	...
	private static InputStream getInputStreamOfJsonSchema() {
		...
		try {
			inputStream = DeleteJobAssigner.class.getClassLoader()
					.getResourceAsStream(ReportingDBConstants.REPORTING_DB_FOLDER_NAME + JsonSchemaFileName.DELETE_JSON_SCHEMA);
